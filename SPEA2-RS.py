...
class Individual(object):
    ...
    def calculate_objective(self):
        self.objective = func1(self.solution)
...
def calculate_fitness_and_distances(P):
    k_n = int(len(P)**0.5) - 1
    for p in P:
        for q in P:
            if p != q:
                distance = sum([(p.objective[k] - q.objective[k])**2 for k in [1, 2, 3]])**0.5
                p.distances.append(distance)
        p.distances.sort()
        p.fitness = p.raw_fitness + 1/(p.distances[k_n] + 2)

def sort_population(P):
    sorted_P = sorted(P, key=lambda individual: individual.fitness)
    return sorted_P

def trim_archive(A, archivenum):
    while len(A) > archivenum:
        all_distances = []
        for p in A:
            for q in A:
                if p != q:
                    distance = sum([(p.objective[i] - q.objective[i])**2 for i in [1, 2, 3]])
                    p.distances.append(distance)
            p.distances.sort()
            all_distances.append(p.distances)
        ...
        while True: 
            if k_n < archivenum:
                ...
                if len(most_crowded) == 1: 
                    A.pop(most_crowded[0])
                    break
                else:
                    k_n = k_n + 1   
            else:
                random_index = random.randint(1, archivenum - 1)  
                A.pop(random_index)
                break  
    return A
...
def binary_tornament(ind1, ind2):
    if ind1.fitness < ind2.fitness:
        ...
        for i in range(len(ind1.distances)):
            if ind1.distances[i] > ind2.distances[i]:
                return ind1
            elif ind1.distances[i] < ind2.distances[i]:
                return ind2
        return ind1
    else:
        return ind2
...
def make_new_pop(A, pc, pm, popnum, archivenum):
    Q = []
    for i in range(int(popnum / 2)):
        i = random.randint(0, archivenum - 1)
        j = random.randint(0, archivenum - 1)
        parent1 = binary_tornament(A[i], A[j])

        m = random.randint(0, archivenum - 1)
        n = random.randint(0, archivenum - 1)
        parent2 = binary_tornament(A[m], A[n])
        
        Two_offspring = crossover_mutation(parent1, parent2, pc, pm)
...
def main():
    ...
    for i in range(popnum):
        A.append(Individual())
        A[i].solution = random.sample(union_list, poplength)
        A[i].calculate_objective()  
        
    for gen_cur in range(generations):
        P = P + A
        calculate_raw_fitness(P)
        calculate_fitness_and_distances(P)
        P = sort_population(P)
        P = P[:popnum]
        A = [p for p in P if p.fitness < 1]
        if len(A) < archivenum:
            A = A + P[len(A):archivenum]    
        elif len(A) > archivenum:
            A = trim_archive(A, archivenum)  
        P = make_new_pop(A, pc, pm, popnum, archivenum)
...
if __name__ == "__main__":
    ...
    with open('', 'r', encoding='utf-8') as file: # Recommendation results from LLM
        data = json.load(file)
